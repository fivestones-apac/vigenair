...
gcloud config set project fs-vigenair-dev
gcloud services enable cloudresourcemanager.googleapis.com
gcloud auth application-default set-quota-project fs-vigenair-dev
printf "\nINFO - GCP project set to 'fs-vigenair-dev' successfully!\n"

BUCKET_EXISTS=$(gcloud storage ls gs://fs-vigenair-dev-vigenair > /dev/null 2>&1 && echo "true" || echo "false")
if "${BUCKET_EXISTS}"; then
  printf "\nWARN - Bucket 'fs-vigenair-dev-vigenair' already exists. Skipping bucket creation...\n"
else
  gcloud storage buckets create gs://fs-vigenair-dev-vigenair --project=fs-vigenair-dev --location=us --uniform-bucket-level-access
  test $? -eq 0 || exit
  printf "\nINFO - Bucket 'fs-vigenair-dev-vigenair' created successfully in location 'us'!\n"
fi
...
PROJECT_NUMBER=$(gcloud projects describe fs-vigenair-dev --format="value(projectNumber)")
...
for SA in "aiplatform.googleapis.com" "storage.googleapis.com" "eventarc.googleapis.com"; do
    gcloud --no-user-output-enabled beta services identity create --project=fs-vigenair-dev \
        --service="${SA}"
done
COMPUTE_SA_ROLES=(
...
--trigger-event-filters="bucket=fs-vigenair-dev-vigenair" \
--trigger-location="us"
test $? -eq 0 || exit
